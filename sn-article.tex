...
The optimized architecture of Sub-Box is shown in Equation~\ref{eq2}, where $X_0-X_3$ is the input of the Sub-Box and $Y_0-Y_3$ is the output of the Sub-Box.
...
In Add Key, the stored data in Mix Columns's registers are sent back to State-Register followed by XORing with keys and through the Sub-Box component in another four clock cycles.
...
The architecture under consideration, as illustrated in Figure~\ref{fig4}, incorporates two Sub-Boxes, two Mix-Columns, a Key, a State-Register, two PermuteNibbles, and a single feedback path. This architecture is engineered to execute a 30-round encryption process within a span of 15 cycles. It is noteworthy that only the Mix-Columns and Add-Key operations are executed in the final cycle, leading to the culmination of the encryption process in a total of 16 cycles.

The unrolled architecture, an offshoot of the iterative architecture expounded in \cite{beierle2019craft}, necessitates a mere 16 cycles for the completion of the encryption process. This stands in stark contrast to the 32 cycles necessitated by its iterative counterpart. In this context, a single cycle is defined as encompassing two round functions of CRAFT. Despite the potential for increased area usage, this methodology yields superior throughput at an equivalent frequency.
...
The Power metric, which encapsulates both dynamic and static power consumption, serves as the evaluative measure for the power consumption of the proposed designs, as delineated in Equation~\ref{eq7}.
Conversely, the Energy metric is employed to gauge the energy consumption of the designs. This is computed by the multiplication of power consumption and the duration required to encrypt a singular block. This duration is ascertained by the division of latency by the frequency, as explicated in Equation~\ref{enery_eqation}.
