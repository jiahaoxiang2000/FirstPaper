...
\begin{table}[h]
    ...
    \begin{tabular}{lccc}
        \toprule
        Architecture & Cipher & Reference & Description  \\
        \midrule
        A1  & CRAFT & This work & Serial \\
        A2  & CRAFT & This work & Unrolled \\
        A3  & PRESENT & \cite{DBLP:journals/tcas/Lara-NinoDM17} & Iterative \\
        \bottomrule
    \end{tabular}
\end{table}
...
The proposed architectures were implemented on a Xilinx FPGA board using Vivado v2023.2. To test in various environments, three different FPGA platforms were used for benchmarking: Artix-7(xc7a100tcsg324-1), Spartan-7(xc7s100fgga484-1), and Kintex-7(xc7k70tfbg484-1).
...
This section presents the results of the proposed designs, divided into three categories: area, throughput, and power and energy. The results are demonstrated across three different FPGA platforms: Artix-7, Spartan-7, and Kintex-7. The area consumption of the designs is displayed in Table~\ref{area_compare}, the throughput results are illustrated in Table~\ref{throughput_compare}, and the details of power and energy consumption are provided in Table~\ref{power_energy_compare}.
...
\begin{table*}
    \begin{threeparttable}
        ...
        \begin{tabular*}{\textwidth}{@{\extracolsep\fill}lcccccccc}
            \toprule
            Platform & Design & $Latency$ & $FMAX(MHz)$ & $Thr(Mbps)$ & $Thr^*(Mbps)$\tnote{a} & $\frac{Thr}{Slices}$($\frac{Kbps}{Slices}$) \\
            \midrule
...
\begin{figure}
    ...
    \caption{Comparison of Flip-Flop in Architectures A1, A2, and A3}\label{compare_ff}
\end{figure}

When it comes to Look-Up Tables (LUT), the proposed designs (A1) require fewer LUTs than the iterative architecture (A3) of PRESENT. This is attributed to the fact that the proposed designs utilize a single Sub-Box, in contrast to the 16 Sub-Boxes used by the iterative architecture of PRESENT. Furthermore, the proposed designs also require fewer LUTs than the unrolled architecture of CRAFT, which uses 32 Sub-Boxes, compared to just one in the proposed designs. A detailed comparison of LUT counts is illustrated in Figure~\ref{compare_lut}.

\begin{figure}
    \centering
    \includegraphics[width=0.45\textwidth]{compare-lut.pdf}
    \caption{Comparison of Look-Up Tables in Architectures A1, A2, and A3}\label{compare_lut}
\end{figure}
